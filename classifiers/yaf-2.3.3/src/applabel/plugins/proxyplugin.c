    ...
    uint8_t type)
{
    ...
    if (type == 1) {
...
#if YAF_ENABLE_HOOKS
        yfHookScanPayload(flow, payload, cipher_suite_len, NULL, offsetptr+39,
                          91, TLS_PORT_NUMBER);
#endif
        ...
        /* cipher is here */
#if YAF_ENABLE_HOOKS
        yfHookScanPayload(flow, payload, 2, NULL, offsetptr+39, 89,
                          TLS_PORT_NUMBER);
#endif
        offsetptr += 2;
        /* compression method */
#if YAF_ENABLE_HOOKS
        yfHookScanPayload(flow, payload, 1, NULL, offsetptr+39, 90,
                          TLS_PORT_NUMBER);
#endif
        offsetptr++;
            ...
            while (payloadSize > offsetptr) {
                ...
                if (cert_count < MAX_CERTS) {
                    ...
                    if ((offsetptr + sub_cert_len + 3) < payloadSize) {
                        yfHookScanPayload(flow, payload, 1, NULL, offsetptr+39,
                                          93, TLS_PORT_NUMBER);
                    }
#endif
                } else {
...
#if YAF_ENABLE_HOOKS
    yfHookScanPayload(flow, payload, cipher_spec_length, NULL, offsetptr+39, 92,
                      TLS_PORT_NUMBER);
#endif
...
static uint16_t yfProxyScanInit()
{
    ...
    int errorPos;
    const char httpRegexString[] = "^CONNECT [a-zA-Z0-9.]+:\\d+ HTTP/\\d\\.\\d\\b";
